...
public class WXPayReport {
    ...
    private WXPayReport(final WXPayConfig config) {
        ...
        if (config.shouldAutoReport()) {
            LogUtils.info("report worker num: {"+config.getReportWorkerNum()+"}");
                    ...
                    public void run() {
                        while (true) {
                            ...
                            try {
                                ...
                                LogUtils.warn("report fail. reason: {"+ex.getMessage()+"}");
                            }
                        }
                    }
                });
