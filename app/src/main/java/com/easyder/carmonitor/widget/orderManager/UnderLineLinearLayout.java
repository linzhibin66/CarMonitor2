...
public class UnderLineLinearLayout extends LinearLayout {
    ...
    public UnderLineLinearLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        lineStrokeWidth = attr.getDimensionPixelOffset(R.styleable.UnderLineLinearLayout_line_stroke_width, 1);
        lineColor = attr.getColor(R.styleable.UnderLineLinearLayout_line_color, 0xFFE8E9ED);
        pointSize = attr.getDimensionPixelSize(R.styleable.UnderLineLinearLayout_point_size, UIUtils.dip2px(3));
        pointColor = attr.getColor(R.styleable.UnderLineLinearLayout_point_color, 0xFFE8E9ED);
        lineGravity = attr.getInt(R.styleable.UnderLineLinearLayout_line_gravity, GRAVITY_LEFT);

        int iconRes = attr.getResourceId(R.styleable.UnderLineLinearLayout_icon_src, R.mipmap.progress_start_icon);
    ...
    private void drawFirstChildViewVertical(Canvas canvas) {
        ...
        if (getChildAt(0) != null) {
            ...
            //记录值
            firstX = (sideRelative>=rootMiddle?(sideRelative-lineMarginSide):(sideRelative+lineMarginSide)) - (mIcon
                    .getWidth() >> 1);
            firstY = top + getChildAt(0).getPaddingTop() + lineDynamicDimen-(mIcon.getWidth() >> 1);
    ...
    private void drawLinrVertical(Canvas canvas){
        ...
        //画剩下的
        canvas.drawLine(lastX, firstY + (mIcon.getWidth() >> 1), lastX, lastY, linePaint);
    }

    private void drawBetweenChildVertical(Canvas canvas) {
